    ...
    return json.dumps({
        ...
        "plugins": {
            "SplashScreen": {
                ...
                "androidScaleType": "FIT_CENTER", "showSpinner": False
            }
        }
    }, indent=2)
...
android {{
    ...
    compileSdk 34
    defaultConfig {{
        applicationId "{c['app_id']}"
        minSdk 22; targetSdk 34
        versionCode 1; versionName "{c['app_version']}"
    }}
...
def generate_firebase_config_js():
    c = CONFIG
    return f'''const firebaseConfig = {{
    apiKey: "{c['firebase_apiKey']}",
    authDomain: "{c['firebase_authDomain']}",
    databaseURL: "{c['firebase_databaseURL']}",
    projectId: "{c['firebase_projectId']}",
    storageBucket: "{c['firebase_storageBucket']}",
    messagingSenderId: "{c['firebase_messagingSenderId']}",
    appId: "{c['firebase_appId']}",
    measurementId: "{c['firebase_measurementId']}"
}};
firebase.initializeApp(firebaseConfig);
const auth = firebase.auth();
const db = firebase.database();
const CLOUD_NAME = "{c['cloudinary_cloud_name']}";
const UPLOAD_PRESET = "{c['cloudinary_upload_preset']}";
const ADMIN_EMAILS = {json.dumps(c['admin_emails'])};
...
body{{font-family:'Segoe UI',sans-serif;background:{c['app_color']};color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center}}
h1{{background:linear-gradient(to bottom,#fff,{c['primary_color']});-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-size:48px}}
</style></head><body><div>
<h1>👑 {c['app_name']}</h1><p>v{c['app_version']}</p>
<p>🃏 Joker Scraper</p></div></body></html>'''
...
def main():
    ...
    write(f"{base}/AndroidManifest.xml", generate_android_manifest())
    write(f"{base}/res/xml/network_security_config.xml", generate_network_config())
    write(f"{base}/res/xml/file_paths.xml", generate_file_paths())
    write(f"{base}/res/values/styles.xml", generate_styles())
    write(f"{base}/res/values/colors.xml", generate_colors())
    write(f"{base}/res/values/strings.xml", generate_strings())
    write(f"{base}/res/drawable/splash_background.xml", generate_splash())
    write(f"{base}/res/drawable/ic_launcher_background.xml", generate_ic_launcher_bg())
    write(f"{base}/res/mipmap-anydpi-v26/ic_launcher.xml", generate_adaptive_icon())
    write(f"{base}/res/mipmap-anydpi-v26/ic_launcher_round.xml", generate_adaptive_icon())
    ...
    # Copy to www
    www = os.path.join(OUTPUT_DIR, "www")
    os.makedirs(www, exist_ok=True)
    for f in ["firebase-config.js", "index.html"]:
        src = os.path.join(OUTPUT_DIR, f)
        if os.path.exists(src):
            shutil.copy2(src, os.path.join(www, f))
    
    # Copy www to root for deployment
    for f in os.listdir(www):
        shutil.copy2(os.path.join(www, f), f)
    
    total = sum(1 for _ in os.walk(OUTPUT_DIR) for __ in _[2])
